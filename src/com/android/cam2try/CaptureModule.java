...
import com.android.cam2try.app.CameraAppUI.BottomBarUISpec;
import com.android.cam2try.app.LocationManager;
import com.android.cam2try.app.OrientationManager.DeviceOrientation;
...
import com.android.cam2try.burst.BurstReadyStateChangeListener;
import com.android.cam2try.burst.OrientationLockController;
import com.android.cam2try.captureintent.PreviewTransformCalculator;
import com.android.cam2try.debug.DebugPropertyHelper;
import com.android.cam2try.debug.Log;
import com.android.cam2try.debug.Log.Tag;
import com.android.cam2try.device.CameraId;
import com.android.cam2try.hardware.HardwareSpec;
import com.android.cam2try.hardware.HeadingSensor;
import com.android.cam2try.module.ModuleController;
import com.android.cam2try.one.OneCamera;
import com.android.cam2try.one.OneCamera.AutoFocusState;
import com.android.cam2try.one.OneCamera.CaptureReadyCallback;
import com.android.cam2try.one.OneCamera.Facing;
import com.android.cam2try.one.OneCamera.OpenCallback;
import com.android.cam2try.one.OneCamera.PhotoCaptureParameters;
...
import com.android.cam2try.remote.RemoteCameraModule;
...
import com.android.cam2try.stats.CaptureStats;
...
import com.android.cam2try.ui.CountDownView;
...
import com.android.cam2try.ui.focus.FocusSound;
import com.android.cam2try.util.AndroidServices;
        ...
        RemoteCameraModule
{
    ...
    /**
     * Timeout for cam2try open/close operations.
     */
    ...
    /**
     * The cam2try manager used to open cameras.
     */
    private OneCameraOpener mOneCameraOpener;
    /**
     * The manager to query for cam2try device information
     */
    private OneCameraManager mOneCameraManager;
    /**
     * The currently opened cam2try device, or null if the cam2try is closed.
     */
    ...
    /**
     * Fair semaphore held when opening or closing the cam2try.
     */
    private final Semaphore mCameraOpenCloseLock = new Semaphore(1, true);
    /**
     * The direction the currently opened cam2try is facing to.
     */
    ...
    /**
     * Handler thread for cam2try-related operations.
     */
    ...
    /**
     * Current width of preview frames from cam2try.
     */
    private int mPreviewBufferWidth;
    /**
     * Current height of preview frames from cam2try..
     */
    ...
    public void resume()
    {
        ...
        // TODO: Check if we can really take a photo right now (memory, cam2try
        // state, ... ).
        ...
        // have to open the cam2try and start the preview here.
    ...
    {
        // Ignore since we manage the cam2try ourselves until we remove this.
    }
    ...
    public void hardResetSettings(SettingsManager settingsManager)
    {
        ...
        {
            // Sticky HDR+ mode should hard reset HDR+ to on, and cam2try back
            // facing.
        ...
        if ((hardwareSpec.isHdrPlusSupported() &&
                ...
                (hardwareSpec.isHdrSupported() &&
                        mAppController.getSettingsManager().getBoolean(
                                SettingsManager.SCOPE_GLOBAL, Keys.KEY_CAMERA_HDR)))
        {
            // Disable flash if this is a sticky gcam cam2try, or if
            // HDR is enabled.
    ...
    private void startActiveFocusAt(int viewX, int viewY)
    {
        if (mCamera == null)
        {
            // If we receive this after the cam2try is closed, do nothing.
            return;
        }
        ...
        // Invert X coordinate on front cam2try since the display is mirrored.
        ...
        } else
        {
            return new ButtonManager.ButtonCallback()
            {
                ...
                public void onStateChanged(int hdrEnabled)
                {
                    ...
                    // Only reload the cam2try if we are toggling HDR+.
     ...
     * returns the proper callback to be used for when the cam2try
        ...
        if (mStickyGcamCamera)
        {
            return new ButtonManager.ButtonCallback()
            {
                ...
                public void onStateChanged(int state)
                {
                    ...
                    // At the time this callback is fired, the cam2try id setting
                    // has changed to the desired cam2try.
                    SettingsManager settingsManager = mAppController.getSettingsManager();
                    if (Keys.isCameraBackFacing(settingsManager,
                            mAppController.getModuleScope()))
                    {
                        throw new IllegalStateException(
                                "Hdr plus should never be switching from front facing cam2try.");
                    }

                    // Switch to photo mode, but request a return to hdr plus on
                    // switching to back cam2try again.
                    settingsManager.set(mAppController.getModuleScope(),
        ...
        } else
        {
            return new ButtonManager.ButtonCallback()
            {
                ...
                public void onStateChanged(int cameraId)
                {
                    ...
                    // At the time this callback is fired, the cam2try id
                    // has be set to the desired cam2try.
                    mSettingsManager.set(mAppController.getModuleScope(), Keys.KEY_CAMERA_ID,
                            cameraId);

                    Log.d(TAG, "Start to switch cam2try. cameraId=" + cameraId);
    ...
    /**
     * Open cam2try and start the preview.
     */
    private void openCameraAndStartPreview()
    {
        ...
        try
        {
            ...
            {
                throw new RuntimeException("Time out waiting to acquire cam2try-open lock.");
            }
        } catch (InterruptedException e)
        {
            throw new RuntimeException("Interrupted while waiting to acquire cam2try-open lock.", e);
        }
        ...
        if (mCamera != null)
        {
            // If the cam2try is already open, do nothing.
        ...
        mOneCameraOpener.open(cameraId, captureSetting, mCameraHandler, mainThread,
                ...
                new OpenCallback()
                {
                    ...
                    {
                        Log.e(TAG, "Could not open cam2try.");
                        ...
                        // cam2try device without showing the error dialog.
                        // Application will properly reopen the cam2try on the next
                    ...
                    public void onCameraOpened(@Nonnull final OneCamera camera)
                    {
                        ...
                        // A race condition exists where the cam2try may be in the process
                        ...
                        // When cam2try is opened, the zoom is implicitly reset to 1.0f
                        mZoomValue = 1.0f;
                        ...
                        camera.startPreview(new Surface(getPreviewSurfaceTexture()),
                                new CaptureReadyCallback()
                                {
                                    ...
                                    public void onSetupFailed()
                                    {
                                        ...
                                        // getting ready to close the cam2try.
                                        mCameraOpenCloseLock.release();
    ...
    /**
     * Re-initialize the cam2try if e.g. the HDR mode or facing property changed.
     */
    ...
    /**
     * Returns which way around the cam2try is facing, based on it's ID.
     */
