...
/**
 * A class which implements {@link com.android.cam2try.app.CameraProvider} used
 * by {@link com.android.cam2try.CameraActivity}.
...
public class CameraController implements CameraAgent.CameraOpenCallback, CameraProvider
{
     ...
     *                                  cam2try is disabled.
     * @param handler                   The {@link android.os.Handler} to post the cam2try
     *                                  callbacks to.
     * @param cameraManager             Used for cam2try open/close.
     * @param cameraManagerNg           Used for cam2try open/close with the new API. If
    ...
    public void requestCamera(int id, boolean useNewApi)
    {
        ...
        // (T, F): Already requested the same cam2try. No-op. Return.
        ...
        } else if (mCameraProxy.getCameraId() != id || mUsingNewApi != useNewApi)
        {
            boolean syncClose = GservicesHelper.useCamera2ApiThroughPortabilityLayer(mContext
                    .getContentResolver());
            Log.v(TAG, "different cam2try already opened, closing then reopening");
            // Already has cam2try opened, and is switching cameras and/or APIs.
            if (mUsingNewApi)
            {
        ...
        {
            // The same cam2try, just do a reconnect.
            Log.v(TAG, "reconnecting to use the existing cam2try");
    ...
    public void releaseCamera(int id)
    {
        if (mCameraProxy == null)
        {
            if (mRequestingCameraId == EMPTY_REQUEST)
            {
                ...
                Log.w(TAG, "Trying to release the cam2try before requesting");
            }
        ...
        if (currentId != id)
        {
            ...
            {
                Log.w(TAG, "Releasing cam2try which was requested but not yet "
                        + "opened (current:requested): " + currentId + ":" + id);
            } else
            {
                throw new IllegalStateException("Trying to release a cam2try neither opened"
