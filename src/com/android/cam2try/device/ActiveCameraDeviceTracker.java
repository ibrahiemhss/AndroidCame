...
/**
 * Shared object for tracking the active cam2try device across multiple
 * implementations.
 */
...
public class ActiveCameraDeviceTracker
{
    ...
    public CameraId getActiveOrPreviousCamera()
    {
        synchronized (mLock)
        {
            ...
            Log.v(TAG, "Returning previously active cam2try: " + mPreviousCamera);
    ...
    public void onCameraOpening(CameraId key)
    {
        synchronized (mLock)
        {
            ...
            Log.v(TAG, "Tracking active cam2try: " + mActiveCamera);
            mActiveCamera = key;
        }
    }
