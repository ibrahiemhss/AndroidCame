...
import com.android.cam2try.device.CameraDeviceKey.ApiType;
...
/**
 * This class's only job is to open and close cam2try devices safely, such that
 ...
 * <p>
 * The logic for opening a cam2try device proceeds as follows:
 * <p>
 * 1. If there is no open cam2try, create a new cam2try request, and open
 * the device.
...
public class MultiCameraDeviceLifecycle
{
    ...
    /**
     * !!! Warning !!!
     * Code using this class should close the cam2try device by closing the
     * provided lifetime instead of calling close directly on the cam2try
     * object. Failing to do so may leave the multi cam2try lifecycle in an
     * inconsistent state.
     * <p>
     * Returns a future to an API2 Camera device. The future will only
     * complete if the cam2try is fully opened successfully. If the device cannot
     ...
     *                        cause the cam2try to close. Closing the lifetime instead of the device
     *                        will ensure everything is shut down properly.
     * @param cameraId        the specific cam2try device to open.
     */
