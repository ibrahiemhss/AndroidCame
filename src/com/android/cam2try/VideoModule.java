...
import com.android.cam2try.settings.SettingsUtil;
        ...
        OrientationManager.OnOrientationChangeListener, VideoController
{
    ...
    // The video file that the hardware cam2try is about to record into
    ...
    /**
     * @return Whether the currently active cam2try is back-facing.
     */
    ...
    /**
     * The focus manager gets initialized after cam2try is available.
     */
    ...
    {
        // TODO: Remove this when old cam2try controls are removed from the UI.
    }
     ...
     * This function should be called after every change in preview cam2try
     ...
     * resolution.  Does nothing when cam2try pointer is null.
     */
     ...
     * com.android.cam2try.cameradevice.CameraCapabilities#getPreferredPreviewSizeForVideo()}
     ...
     *
     * @return The preferred preview size or {@code null} if the cam2try is not
     * opened yet.
     */
    ...
    private void initializeRecorder()
    {
        ...
        if (mCameraDevice == null)
        {
            Log.w(TAG, "null cam2try proxy, not recording");
            return;
        }
        Intent intent = mActivity.getIntent();
        ...
        // Unlock the cam2try object before passing it to media recorder.
        ...
        // If the cam2try device is null, the cam2try proxy is stale and recording
        // should be ignored.
        if (camera == null)
        {
            Log.w(TAG, "null cam2try within proxy, not recording");
            return;
        }
        ...
        mActivity.updateStorageSpaceAndHint(new CameraActivity.OnStorageUpdateDoneListener()
        {
            ...
            public void onStorageUpdateDone(long bytes)
            {
                ...
                } else
                {
                    if (mCameraDevice == null)
                    {
                        Log.v(TAG, "in storage callback after cam2try closed");
                        return;
                    }
                    if (mPaused == true)
                    {
                    ...
                    } catch (RuntimeException e)
                    {
                        ...
                        releaseMediaRecorder();
                        // If start fails, frameworks will not lock the cam2try for us.
                        mCameraDevice.lock();
                        return;
                    }
    ...
    private Bitmap getVideoThumbnail()
    {
        ...
        if (bitmap != null)
        {
            ...
            // it to match the UI orientation (and mirror if it is front-facing cam2try).
    ...
    {
        // Do nothing if cam2try device is still capturing photo. Monkey test can trigger app crashes
        ...
        if (mMediaRecorderRecording)
        {
            ...
            // during recording. Release the cam2try as soon as possible because
            // face unlock or other applications may need to use the cam2try.
            if (mPaused)
            {
    ...
    private void setCameraParameters()
    {
        ...
        // Update Desired Preview size in case video cam2try resolution has changed.
        updateDesiredPreviewSize();
        ...
        // Set picture size.
        // The logic here is different from the logic in still-mode cam2try.
    ...
    public void resume()
    {
        ...
        if (mFocusManager != null)
        {
            // If cam2try is not open when resume is called, focus manager will not
    ...
    private void switchCamera()
    {
        ...
        Log.d(TAG, "Start to switch cam2try.");
