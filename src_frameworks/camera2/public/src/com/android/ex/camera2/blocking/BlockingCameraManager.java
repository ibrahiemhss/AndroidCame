...
public class BlockingCameraManager {
    ...
    public static class BlockingOpenException extends Exception {
        ...
        /**
         * Thrown when cam2try device enters error state during open, or if
         * it disconnects.
         *
         * @param errorCode
         * @param message
    ...
    /**
     * Create a new blocking cam2try manager.
     *
     * @param manager
    ...
    /**
     * Open the cam2try, blocking it until it succeeds or fails.
     *
     ...
     * highly unrecoverable, and all future calls to opening that cam2try will fail since the
     * service will think it's busy. This class will do its best to clean up eventually.</p>
     *
     * @param cameraId
     *            Id of the cam2try
     * @param listener
     *            Listener to the cam2try. onOpened, onDisconnected, onError need not be implemented.
     * @param handler
     ...
     *
     * <p>Time out after {@link #OPEN_TIME_OUT_MS} and unblock. Clean up cam2try if it arrives
     * later.</p>
     */
    private class OpenListener extends CameraDevice.StateCallback {
        ...
        public void onOpened(CameraDevice camera) {
            if (VERBOSE) {
                Log.v(TAG, "onOpened: cam2try " + ((camera != null) ? camera.getId() : "null"));
            }
        ...
        public void onDisconnected(CameraDevice camera) {
            if (VERBOSE) {
                Log.v(TAG, "onDisconnected: cam2try "
                        + ((camera != null) ? camera.getId() : "null"));
            }

            synchronized (mLock) {
                // Don't assert all initial states. onDisconnected can be called after cam2try
        ...
        public void onError(CameraDevice camera, int error) {
            if (VERBOSE) {
                Log.v(TAG, "onError: cam2try " + ((camera != null) ? camera.getId() : "null"));
            }
        ...
        CameraDevice blockUntilOpen() throws BlockingOpenException {
            ...
            if (!mDeviceReady.block(OPEN_TIME_OUT_MS)) {
                ...
                synchronized (mLock) {
                    if (mNoReply) { // Give the async cam2try a fighting chance (required)
                        mTimedOut = true; // Clean up cam2try if it ever arrives later
                        throw new TimeoutRuntimeException(String.format(
                                "Timed out after %d ms while trying to open cam2try device %s",
                ...
                if (mSuccess) {
                    ...
                    if (mDisconnected) {
                        throw new BlockingOpenException(
                                ...
                                "Failed to open cam2try device: it is disconnected");
                    } else if (mError != ERROR_UNINITIALIZED) {
                        throw new BlockingOpenException(
                                mError,
                                "Failed to open cam2try device: error code " + mError);
                    } else {
                        throw new AssertionError("Failed to open cam2try device (impl bug)");
                    }
                }
            }
        }
