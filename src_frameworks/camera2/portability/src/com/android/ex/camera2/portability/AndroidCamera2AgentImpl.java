...
class AndroidCamera2AgentImpl extends CameraAgent {
    ...
    /**
     * Number of cam2try devices.  The length of {@code mCameraDevices} does not reveal this
     * information because that list may contain since-invalidated indices.
     */
    private int mNumCameraDevices;

    /**
     * Transformation between integral cam2try indices and the {@link java.lang.String} indices used
    ...
    /**
     * Updates the cam2try device index assignments stored in {@link mCameraDevices}, without
    ...
    private boolean updateCameraDevices() {
        try {
            ...
            // Invalidate the indices assigned to any cam2try devices that are no longer present
            ...
            // Assign fresh indices to any new cam2try devices
            ...
            Log.e(TAG, "Could not get device listing from cam2try subsystem", ex);
            return false;
        }
    }
    ...
    private class Camera2Handler extends HistoryHandler {
            ...
            try {
                switch (cameraAction) {
                    ...
                    case CameraActions.RECONNECT: {
                        ...
                        Log.i(TAG, String.format("Opening cam2try index %d (id %s) with camera2 API",
                    ...
                    case CameraActions.AUTO_FOCUS: {
                        ...
                        // Send a one-time capture to trigger the cam2try driver to lock focus.
                    ...
                    case CameraActions.CANCEL_AUTO_FOCUS: {
                        ...
                        // Send a one-time capture to trigger the cam2try driver to resume scanning.
                        changeState(AndroidCamera2StateHolder.CAMERA_PREVIEW_ACTIVE);
                    ...
                    case CameraActions.CAPTURE_PHOTO: {
                        ...
                        if (mLegacyDevice ||
                            ...
                            // Set a one-time capture to trigger the cam2try driver's autoexposure:
        ...
        public CameraSettings buildSettings(AndroidCamera2Capabilities caps) {
            try {
                ...
                Log.e(TAG, "Unable to query cam2try device to build settings representation");
                return null;
            }
        }
        ...
        private void setPreviewTexture(SurfaceTexture surfaceTexture) {
            ...
            try {
                ...
                Log.e(TAG, "Failed to create cam2try capture session", ex);
            }
        }
        ...
        private void closePreviewSession() {
            try {
                ...
                Log.e(TAG, "Failed to close existing cam2try capture session", ex);
            }
            changeState(AndroidCamera2StateHolder.CAMERA_CONFIGURED);
        }
        ...
        // This callback monitors our connection to and disconnection from cam2try devices.
        ...
        // This callback monitors our cam2try session (i.e. our transition into and out of preview).
        private CameraCaptureSession.StateCallback mCameraPreviewStateCallback =
            ...
            public void onConfigureFailed(CameraCaptureSession session) {
                ...
                Log.e(TAG, "Failed to configure the cam2try for capture");
            }
    ...
    private static class AndroidCamera2StateHolder extends CameraStateHolder {
        ...
        /* Camera states */
        /** No cam2try device is opened. */
        public static final int CAMERA_UNOPENED = 1 << 0;
        /** A cam2try is opened, but no settings have been provided. */
        public static final int CAMERA_UNCONFIGURED = 1 << 1;
        /** The open cam2try has been configured by providing it with settings. */
    ...
    private static class AndroidCamera2DeviceInfo implements CameraDeviceInfo {
        ...
        public AndroidCamera2DeviceInfo(CameraManager cameraManager,
            ...
            for (int id = 0; id < cameraIds.length; ++id) {
                try {
                    ...
                    Log.w(TAG, "Couldn't get characteristics of cam2try '" + id + "'", ex);
                }
            }
        ...
        private static class AndroidCharacteristics2 extends Characteristics {
            ...
            public Matrix getPreviewTransform(int currentDisplayOrientation,
                ...
                // calculated, since that is not done by the new cam2try API.
